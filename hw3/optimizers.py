...
class VanillaSGD(Optimizer):
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            # Use += and -= operators to actually update p and dp value in self.params:
            dp += p * self.reg
...
class MomentumSGD(Optimizer):
    ...
    def step(self):
        for i, (p, dp) in enumerate(self.params):
            ...
            # ====== YOUR CODE: ======
            dp += p * self.reg
            self.velocities[i] = self.velocities[i] *self.momentum - dp*self.learn_rate
            p += self.velocities[i]
            # ========================
...
class RMSProp(Optimizer):
    ...
    def step(self):
        for i, (p, dp) in enumerate(self.params):
            ...
            p -= (self.learn_rate / (torch.sqrt(self.r[i] + self.eps))) * dp
            # ========================
