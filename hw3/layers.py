...
class Sigmoid(Layer):
    ...
    def backward(self, dout):
        ...
        # TODO: Implement gradient w.r.t. the input x
        # ====== YOUR CODE: ======
        exponent = self.grad_cache["exp"]
        dx = dout*(exponent/((1+exponent)**2))
        # ========================
...
class TanH(Layer):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        exponent = torch.exp(x)
        neg_exponent = torch.exp(-x)
        self.grad_cache["exp"] = exponent
        self.grad_cache["neg_exponent"] = neg_exponent
        out = (exponent-neg_exponent)/(exponent+neg_exponent)
        # ========================
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        exponent = self.grad_cache["exp"]
        neg_exponent = self.grad_cache["neg_exponent"]
        dx = dout * (4 / ((neg_exponent + exponent) ** 2))
        # ========================
...
class Linear(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        self.db += torch.squeeze(dout.T @ torch.ones(size=(dout.size()[0], 1)))
        self.dw += dout.T @ x
        dx = dout @ self.w
        # ========================
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        # ====== YOUR CODE: ======
        x_y = torch.squeeze(torch.gather(x, dim=1, index=torch.unsqueeze(y,dim=1)))
        exponent = torch.exp(x)
    ...
    def backward(self, dout=1.0):
        ...
        exponent = torch.exp(x)
        sum = torch.sum(exponent, dim=1)
        qi = exponent / sum[:, None]
        ones = torch.ones(size=(N, 1))
        ind_matrix = torch.zeros(size=x.size()).scatter(1, index=torch.unsqueeze(y, dim=1), src=ones)
        dx = dout * (qi - ind_matrix) / N
        # ========================
...
class Sequential(Layer):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        # Assign out to be the input
        out = x
        # Loop over all layers and forward the input
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        din = dout
        # Loop over all layers in reverse order and back propagate
